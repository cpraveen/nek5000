...
c     %-----------------------------%
c
      integer           lx1, ly1, lz1, lelt, ltot1
      parameter         (lx1=14, ly1=lx1, lz1=1, lelt=36,
     &                   ltot1=lx1*ly1*lz1*lelt)
      integer           maxn, maxnev, maxncv, ldv
      parameter         (maxn=2*ltot1, maxnev=50, maxncv=200, 
     &                   ldv=maxn )
c
c     %--------------%
c     | Local Arrays |
...
c
      intrinsic         abs
c
      double precision  dt, dumm, dreal, dimag
      integer           nsteps
c     %-----------------------%
...
c
            ...
            dumm = 1.0/(dt*nsteps)
            do j=1, iparam(5)
               dreal  = dlog(dsqrt(d(j,1)**2+d(j,2)**2))*dumm
               dimag  = datan2(d(j,2),d(j,1))*dumm
               write(*,*) j, dreal, dimag
            enddo
...
c------------------------------------------------------------------------
      subroutine save_eigvec (n, m, maxncv, v)
      integer n, m, maxncv
      double precision
     &        v(n,maxncv)
